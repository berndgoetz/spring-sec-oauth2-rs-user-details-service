...
/**
 * Retrieves the public key for signature validation for PingFederate access tokens
 */
@Profile("ping")
@Component
@Slf4j
public class PingFederateKeyResolver implements SigningKeyResolver {

    private static final String KEY_ID = "kid";
    private final String x509BaseUrl;
    private final RestTemplate keyUriRestTemplate = new RestTemplate();

    public PingFederateKeyResolver(Environment env) {
        this.x509BaseUrl = env.getProperty("app.pingFederateKeyUrl");
    }

    @Override
    @Cacheable(cacheNames = {CachingConfig.SIGNING_KEY_CACHE}, key = "#header.get('kid')", sync = true)
    public Key resolveSigningKey(JwsHeader header, Claims claims) {
        String keyId = (String) header.get(KEY_ID);
        String pk = getKeyFromServer(keyId);
        return extractPublicKey(pk);
    }
